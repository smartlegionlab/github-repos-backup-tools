...
class AppManager:
    ...
    def run(self):
        ...
        if not args_manager_status:
            print('[err] Error! No arguments found...')
            self._exit()

        config_file = self._create_config()

        if not config_file:
            print('[err] Error creating configuration...')
            self._exit()
        ...
        if not all([token_verify_success, github_login]):
            print("[err] Failed to authenticate with GitHub")
        ...
        print(f"[ok] Authenticated as: {github_client.login}")
        ...
        if self.args_manager.args.repos:
            ...
            if not repo_manager_status:
                print(f"[err] Error cloning repositories!\n")
    ...
    def _parse_args(self):
        ...
        if not any([args.repos, args.gists, args.token]):
            ...
            print("\n[err] Error: Specify at least one backup operation (-r or -g or -t)")
            return False
        ...
        print(f"   Verbose: {'[ok] Enabled' if args.verbose else '[err] Disabled'}")

        if args.shutdown:
            print("   Shutdown: [ok] After completion")
        elif args.reboot:
            print("   Reboot: [ok] After completion")
        else:
            print("   Power: [err] No action")
        return True
